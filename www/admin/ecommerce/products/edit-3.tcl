# /www/admin/ecommerce/products/edit-3.tcl
ad_page_contract {
...
db_transaction {
  ...
  foreach old_subsubcategory_id $old_subsubcategory_id_list {
    if { [lsearch -exact $subsubcategory_id_list $old_subsubcategory_id] == -1 } {
      ...
      # audit
      ad_audit_delete_row [list $old_subsubcategory_id $product_id] [list subsubcategory_id product_id] ec_subsubcat_prod_map_audit
    }
  }
  ...
  foreach old_subcategory_id $old_subcategory_id_list {
    if { [lsearch -exact $subcategory_id_list $old_subcategory_id] == -1 } {
      ...
      # audit
      ad_audit_delete_row [list $old_subcategory_id $product_id] [list subcategory_id product_id] ec_subcat_prod_map_audit
    }
  }
  ...
  foreach old_category_id $old_category_id_list {
    if { [lsearch -exact $category_id_list $old_category_id] == -1 } {
      ...
      # audit
      ad_audit_delete_row [list $old_category_id $product_id] [list category_id product_id] ec_category_product_map_audit
    }
  }
  ...
  foreach user_class_id $old_user_class_id_list {
    if { ![info exists user_class_prices($user_class_id)] || [empty_string_p [set user_class_prices($user_class_id)]] } {
      ...
      # audit
      ad_audit_delete_row [list $user_class_id [lindex $old_user_class_price_list $counter] $product_id] [list user_class_id price product_id] ec_product_u_c_prices_audit
    }
    incr counter
  }
