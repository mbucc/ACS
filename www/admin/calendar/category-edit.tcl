# $Id: category-edit.tcl,v 3.0.4.1 2000/04/28 15:08:26 carsten Exp $
...
if [catch { ns_db dml $db "begin transaction" 
    ...
    # if a new row was not inserted, make sure that the exisitng entry is enabled
    if { [ns_ora resultrows $db] == 0 } {
	ns_db dml $db "update calendar_categories set enabled_p = 't' where category = '$QQcategory_new'"
    } 

    # update all the postings
    ns_db dml $db "update calendar set category='$QQcategory_new' where category='$QQcategory'"
    ...
    ad_return_error "Error updating category" "We couldn't update your category. Here is what the database returned:
<p>
<blockquote>
<pre>
$errmsg
