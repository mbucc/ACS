...
ad_library {
  ...
  @author Eve Andersson (eveander@arsdigitia.com)
  @creation-date Summer 1999
  @cvs-id ecommerce-state-changes.tcl,v 3.1.2.5 2001/01/13 23:04:30 kevin Exp
}
...
proc ec_apply_gift_certificate_balance { order_id user_id } {
  ...
  while { $amount_owed > 0 && $gift_certificate_balance > 0 && [llength $certificates_to_use] > $counter} {
    ...
    if { $gift_certificate_amount_left > 0 } {
      ...
      VALUES
      (:gift_certificate_id, :order_id, least(to_number(:gift_certificate_amount_left), to_number(:amount_owed)), sysdate)
      "
