...
#
# $Id: manage-portal-2.tcl,v 3.1.2.1 2000/04/28 15:11:16 carsten Exp $
#

ad_page_variables {left right hiddennames} 
...
foreach left_list $left {
	...
	if { [ llength $right_list] > 0  || [ llength $left_list ] > 0 } {
	    ...
	    ns_db dml $db "
	        ...
	        values
	        ($page_id, $user_id, $i, '[DoubleApos [lindex $hiddennames [expr $i-1]]]')"
	}
    } 
...
if {[empty_string_p $good_list] && ![empty_string_p $page_list] } {
    ...
    ns_db dml $db "
        delete from portal_table_page_map
        where page_id IN ([join $page_list ,])"
}
