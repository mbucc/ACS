ad_page_contract {
    ...
    @cvs-id tcl-proc-view.tcl,v 1.1.2.3 2000/07/27 21:53:44 tnight Exp
} {
    tcl_proc
}
...
set tcl_api_root "http://www.aolserver.com/doc/3.0/tcldev/"

set tcl_proc [lindex $tcl_proc 0] 
if [info exists tcl_api_page($tcl_proc)] {
    ad_returnredirect "$tcl_api_root$tcl_api_page($tcl_proc)"
} else {
    doc_set_property title "Tcl API Procedure Search for: \"$tcl_proc\""
    doc_set_property navbar [list [list "" "API Browser"] "TCL API Search: $tcl_proc"]
