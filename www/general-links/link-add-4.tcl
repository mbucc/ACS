...
#
# $Id: link-add-4.tcl,v 3.1.2.2 2000/04/28 15:10:39 carsten Exp $
#--------------------------------------------------------
...
ad_page_variables {on_which_table on_what_id item {module ""} return_url link_id association_id link_title url rating {category_id_list -multiple-list} {link_description ""}}
...
if [catch {
    ...
    ad_general_link_map_add $db $association_id $link_id $on_which_table $on_what_id $item $user_id $originating_ip $approved_p
	...
	$errmsg
        ...
        return
     } elseif { [database_to_tcl_string $db "select count(*) from site_wide_link_map where map_id = $association_id"] == 0 } { 
