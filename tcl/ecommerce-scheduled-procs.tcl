# $Id: ecommerce-scheduled-procs.tcl,v 3.3 2000/03/12 01:09:03 eveander Exp $
...
# I'll run this nightly so that calculations of popular product combinations don't
# have to be done each time a product's page is accessed.
# I will actually look at all orders, not just orders which have been confirmed, 
# so that there will be more data (so this isn't *technically* a calculation of
# "people who bought this product also bought these products, because the buying
# didn't have to take place, only the placing into the cart).
...
proc ec_calculate_product_purchase_combinations {} {
    ...
    # for each product, I want to find other products that are items of orders with
    # the same user id
    
    set selection [ns_db select $db "select product_id from ec_products"]

    while { [ns_db getrow $db $selection] } {
	set_variables_after_query
	
	set sub_selection [ns_db select $db_sub "select i2.product_id as correlated_product_id, count(*) as n_product_occurrences
	from ec_items i2
	where i2.order_id in (select o2.order_id from ec_orders o2 where o2.user_id in (select user_id from ec_orders o where o.order_id in (select i.order_id from ec_items i where product_id=$product_id)))
	...
	if { [database_to_tcl_string $db_sub "select count(*) from ec_product_purchase_comb where product_id=$product_id"] == 0 } {
	    if { [llength $insert_cols] > 0 } {
		ns_db dml $db_sub "insert into ec_product_purchase_comb
		...
		ns_db dml $db_sub "update ec_product_purchase_comb
...
# this procedure is needed because new order email is only sent after the order is
# authorized and some authorizations occur when the user is not on the web site
# or execution of the thread on the site may terminate after the order is authorized
# but before the email is sent
...
proc ec_delayed_credit_denied {}  {
    ...
    set order_id_list [database_to_tcl_list $db "select order_id from ec_orders where order_state='failed_authorization'"]

    foreach order_id $order_id_list {
	ns_log Notice "working on order #$order_id"

	# save this shopping cart for the user
	ns_db dml $db "update ec_orders set order_state='in_basket', saved_p='t' where order_id=$order_id"
...
proc ec_expire_old_carts {} {
    ...
    ns_db dml $db "update ec_orders set order_state='expired', expired_date=sysdate where order_state='in_basket' and sysdate-in_basket_date > [ad_parameter CartDuration ecommerce]"
    ns_db dml $db "update ec_items set item_state='expired', expired_date=sysdate where item_state='in_basket' and order_id in (select order_id from ec_orders where order_state='expired')"
...
proc ec_remove_creditcard_data {} {
	...
	set db [ns_db gethandle]
	ns_db dml $db "update ec_creditcards set
...
where creditcard_id in (select unique c.creditcard_id
                        ...
                        and 0=(select count(*) from ec_orders o2 where o2.creditcard_id=c.creditcard_id and o2.order_state not in ('fulfilled','returned','void','expired')))"
        ns_db releasehandle $db
    }
}
...
proc ec_unauthorized_transactions {} {
    ...
    set selection [ns_db select $db2 "select transaction_id, order_id from ec_financial_transactions
	...
	if { [regexp {success} [ns_set get $ttcc_output "txn_status"]] } {
	    ...
	    # Retry the authorization
	    set new_cc_status [ec_creditcard_authorization $db $order_id $transaction_id]
		...
		if { $new_cc_status == "invalid_input" } {
		    ...
		    (problem_id, problem_date, problem_details, order_id)
		    values
		    (ec_problem_id_sequence.nextval, sysdate, 'invalid input to ec_creditcard_authorization in ec_unauthorized_transactions', [ns_dbquotevalue $order_id integer])
		    "
		}
	    }
	    ...
	    (problem_id, problem_date, problem_details, order_id)
	    values
	    (ec_problem_id_sequence.nextval, sysdate, 'strange CyberCash result when querying about auth: [DoubleApos [ns_set get $ttcc_output_2 "txn_status"]]', [ns_db quotevalue $order_id])
	    "
	} 
	...
	if { $auth_status_is_now == "success" } {
	    ...
	    (problem_id, problem_date, problem_details, order_id)
	    values
	    (ec_problem_id_sequence.nextval, sysdate, 'The authorization failed for transaction_id $transaction_id.', [ns_dbquotevalue $order_id])
	    "
	    ns_db dml $db "end transaction"
	}
    } 
...
proc ec_unmarked_transactions {} {
	...
	set postauth_success [ec_creditcard_marking $db $transaction_id]
	...
	if { $postauth_success != "success" } {
		...
		(problem_id, problem_date, problem_details, order_id)
		values
		(ec_problem_id_sequence.nextval, sysdate, 'The marking failed for transaction_id $transaction_id.', [ns_dbquotevalue $order_id integer])
		"
		ns_db dml $db "end transaction"
	    }
...
proc ec_unsettled_transactions {} {
	...
	set ttcc_output [ec_talk_to_cybercash "query" $cc_args]
	if { [regexp {success} [ns_set get $ttcc_output "txn_status"]] } {
	    ns_db dml $db "update ec_financial_transactions set settled_date=sysdate where transaction_id=$transaction_id"
	} else {
	    ...
	    if { [database_to_tcl_string $db "select decode(sign(1 - ((sysdate-marked_date)/2)), -1, 1, 0) from ec_financial_transactions where transaction_id=$transaction_id"] } {
		ns_db dml $db "begin transaction"
		ns_db dml $db "update ec_financial_transactions set failed_p='t' where transaction_id=$transaction_id"
		ns_db dml $db "insert into ec_problems_log
		(problem_id, problem_date, problem_details, order_id)
		values
		(ec_problem_id_sequence.nextval, sysdate, 'The settlement failed for transaction_id $transaction_id even though it has been more than 2 days since the transaction was marked.', [ns_dbquotevalue $order_id integer])
		"
		ns_db dml $db "end transaction"
	    }
	}
...
proc ec_unrefunded_transactions {} {
	...
	set return_success [ec_creditcard_return $db $transaction_id]
	...
	if { $return_success != "success" } {
	    ...
	    if { [regexp {success} [ns_set get $ttcc_output "txn_status"]] } {
		ns_db dml $db "update ec_financial_transactions set refunded_date=sysdate where transaction_id=$transaction_id"
	    } else {
		ns_db dml $db "begin transaction"
		ns_db dml $db "update ec_financial_transactions set failed_p='t' where transaction_id=$transaction_id"
		ns_db dml $db "insert into ec_problems_log
		(problem_id, problem_date, problem_details, order_id)
		values
		(ec_problem_id_sequence.nextval, sysdate, 'The refund (the marking of it) failed for transaction_id $transaction_id.', [ns_dbquotevalue $order_id integer])
		"
		ns_db dml $db "end transaction"
	    }
...
proc ec_unrefund_settled_transactions {} {
	...
	set ttcc_output [ec_talk_to_cybercash "query" $cc_args]
	if { [regexp {success} [ns_set get $ttcc_output "txn_status"]] } {
	    ns_db dml $db "update ec_financial_transactions set refund_settled_date=sysdate where transaction_id=$transaction_id"
	} else {
	    # see if it's been at least 2 days since the order was marked (otherwise, don't bother
	    # making a note of the failure yet; in my experience, there were a number of times
	    ...
	    if { [database_to_tcl_string $db "select decode(sign(1 - ((sysdate-refunded_date)/2)), -1, 1, 0) from ec_financial_transactions where transaction_id=$transaction_id"] } {
		ns_db dml $db "begin transaction"
		ns_db dml $db "update ec_financial_transactions set failed_p='t' where transaction_id=$transaction_id"
		ns_db dml $db "insert into ec_problems_log
		(problem_id, problem_date, problem_details, order_id)
		values
		(ec_problem_id_sequence.nextval, sysdate, 'The refund settlement failed for transaction_id $transaction_id even though it has been more than 2 days since the transaction was marked refunded.', $order_id)
		"
		ns_db dml $db "end transaction"
	    }
	}
