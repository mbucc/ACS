...
if { ![empty_string_p $close_date] } {
    ...
    <td><a href=\"/admin/users/one?user_id=$closed_by\">$closed_rep_name</a></td>
    </tr>
    "
}
...
db_foreach get_actions_assoc_w_user $sql {
	...
	<tr bgcolor=\"ececec\"><td>[util_AnsiDatetoPrettyDate [lindex [split $full_interaction_date " "] 0]] [lindex [split $full_interaction_date " "] 1]</td><td>[ec_decode $customer_service_rep "" "&nbsp;" "<a href=\"/admin/users/one?user_id=$customer_service_rep\">$customer_service_rep</a>"]</td><td>$interaction_originator</td><td>$interaction_type</td>
	"
    }
