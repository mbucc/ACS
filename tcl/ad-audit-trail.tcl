...
ad_library {
    ...
    Procedures uses to view the audit trails documented in /www/doc/audit.html 
...
ad_proc ad_audit_trail { 
    {
	-bind {}
    }
    id_list audit_table_name main_table_name id_column_list { columns_not_reported ""} {start_date ""} {end_date ""} {restore_url ""}} {
	
	Returns an HTML fragment showing changes to one row in the OLTP system
	...
	set main_column_name [ad_audit_generate_column_name $main_table_name $id_column]
	set audit_column_name [ad_audit_generate_column_name $audit_table_name $id_column]
    ...
    # start_date is not empty. So display the first record as an update$audit_table_name.$id_column,
    ...
    # ad_audit_process_rows looks for ns_set called bind_vars. Set it
    #  to the bind variables for the audit trail
    # We use db_with_handle because we really have no way of knowing
    #  what is in the rows we're getting back. ad_audit_process_row
    ...
    select 
         $main_table_name.*,  
         users.first_names || ' ' || users.last_name as modifying_user_name, 
         to_char($main_table_name.last_modified,'Mon DD, YYYY HH12:MI AM') as last_modified
...
proc_doc ad_audit_trail_for_table { main_table_name audit_table_name id_column {start_date ""} {end_date ""} {audit_url ""} {restore_url ""}} "Returns the audit trail for each id from the id_column for updates and deletes from main_table_name and audit_table_name that occured between start_date and end_date. If start_date is blank, then it is assumed to be when the table was created, if end_date is blank then it is assumed to be the current time. The audit_url, if it exists, will be given the calling arguments for ad_audit_trail." {
    ...
    set id_list [db_list audit_table_column_select \
	    "select distinct $id_column from $main_table_name where [join $date_clause_list "\nand "]"]
...
<blockquote>
[ad_audit_trail $id $audit_table_name $main_table_name $id_column "" $start_date $end_date]
</blockquote>
"
}
    ...
    set id_list [db_list audit_table_column_select "select distinct $id_column from $audit_table_name where [join $date_clause_list "\nand "] " -bind $bind_vars]
    ns_set free $bind_vars
...
<blockquote>
[ad_audit_trail $id $audit_table_name $main_table_name $id_column "" $start_date $end_date $restore_url]
</blockquote>
"
    }
...
ad_proc ad_audit_generate_column_name { table column { max_length 29 } } {
    Returns a max-length character (max) length identifier for the table and
