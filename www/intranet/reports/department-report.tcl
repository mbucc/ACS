...
set sql_query "select users.user_id, users.last_name || ', ' || users.first_names as name, 
                      (select  im_job_titles.job_title  as current_job_option 
                       ...
                       where im_employee_info.user_id = users.user_id 
                             and im_job_titles.job_title_id = im_employee_info.current_job_id) 
                       as job,
                      (select  im_departments.department 
                       from im_employee_info, im_departments 
                       where im_employee_info.user_id = users.user_id 
                             and im_departments.department_id = im_employee_info.department_id) 
                       as department, start_date, percentage_time
               from users, im_employee_info info, im_employee_percentage_time
...
set context_bar [ad_context_bar_ws [list "[im_url_stub]/reports/" Reports] "Department report"]
