...
foreach usps_abbrev $usps_abbrev_list {
	...
	append exception_text "<li>You forgot to enter the tax rate for [ad_state_name_from_usps_abbrev $usps_abbrev]"
    } 

    if { ![info exists shipping_p($usps_abbrev)] || [empty_string_p $shipping_p($usps_abbrev)] } {
	incr exception_count
	append exception_text "<li>You forgot to specify whether tax is charged for shipping in [ad_state_name_from_usps_abbrev $usps_abbrev]"
    }
...
db_transaction {
	...
	if { [lsearch $usps_abbrev_list $old_state] == -1 } {
	    ...
	    ad_audit_delete_row [list $old_state] [list usps_abbrev] ec_sales_tax_by_state_audit
	}
    }
