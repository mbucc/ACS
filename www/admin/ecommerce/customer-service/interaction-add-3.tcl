...
ad_page_contract {  
    ...
    @author
    @creation-date
    @cvs-id interaction-add-3.tcl,v 3.3.2.13 2001/01/12 19:55:34 khy Exp
} { 
    ...
    action_id:notnull,naturalnum,verify
    close_issue_p
    {issue_type_list "" }
...
if { ![empty_string_p $issue_id] } {
	...
	if { ![empty_string_p $issue_user_id] } {
		...
		<hr>
		Issue ID $issue_id belongs to the registered user <a href=\"/admin/users/one?user_id=$issue_user_id\">[db_string get_full_name "select first_names || ' ' || last_name from users where user_id=:issue_user_id"]</a>.
	...
	if { ![empty_string_p $issue_user_id] } {
		...
		<a href=\"/admin/users/one?user_id=$issue_user_id\">[db_string get_full_name "select first_names || ' ' || last_name from users where user_id=:issue_user_id"]</a>.  However, you haven't associated this interaction with any registered user.  You've associated it with the unregistered user [ec_user_identification_summary $c_user_identification_id].  If these are really the same user, match them up by clicking on the \"user info\" link and then you can reload this page without getting this error message." 
		return
...
if { [info exists order_id] && ![empty_string_p $order_id] } {
	...
	if { ![info exists interaction_id] } {
		...
		<hr>
		Order ID $order_id belongs to the registered user <a href=\"/admin/users/one?user_id=$order_user_id\">[db_string get_user_name "select first_names || ' ' || last_name from users where user_id=:order_user_id"]</a>.
		...
		<a href=\"/admin/users/one?user_id=$order_user_id\">[db_stringget_user_full_name "select first_names || ' ' || last_name from users where user_id=:order_user_id"]</a>.  However, you haven't associated this interaction with any registered user.  You've associated it with the unregistered user [ec_user_identification_summary $c_user_identification_id].  If these are really the same user, match them up by clicking on the \"user info\" link and then you can reload this page without getting this error message." 
		return
