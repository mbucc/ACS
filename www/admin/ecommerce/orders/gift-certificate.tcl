# /www/admin/ecommerce/orders/gift-certificate.tcl
ad_page_contract {
...
[ad_admin_context_bar [list "../" "Ecommerce"] [list "index" "Orders"] [list "gift-certificates" "Gift Certificates"] "One"]
...
if { ![empty_string_p $issuer_user_id] } {
    doc_body_append "<tr><td>Issued By</td><td><a href=\"/admin/users/one?user_id=$issuer_user_id\">$issuer</a> on [util_AnsiDatetoPrettyDate $issue_date]</td></tr>
    <tr><td>Issued To</td><td><a href=\"/admin/users/one?user_id=$user_id\">$owned_by</a></td><tr>"
} else {
    doc_body_append "<tr><td>Purchased By</td><td><a href=\"/admin/users/one?user_id=$purchaser_user_id\">$purchaser</a> on [util_AnsiDatetoPrettyDate $issue_date]</td></tr>
    <tr><td>Sent To</td><td>$recipient_email</td></tr>
    "

    if { ![empty_string_p $user_id] } {
	doc_body_append "<tr><td>Claimed By</td><td><a href=\"/admin/users/one?user_id=$user_id\">$owned_by</a> on [util_AnsiDatetoPrettyDate $claimed_date]</td></tr>"
    }
...
if { $gift_certificate_state == "void" } {
    doc_body_append "<tr><td><font color=red>Voided</font></td><td>[util_AnsiDatetoPrettyDate $voided_date] by <a href=\"/admin/users/one?user_id=$voided_by\">$voided_by_first_names $voided_by_last_name</a> because: $reason_for_void</td></tr>"
}
