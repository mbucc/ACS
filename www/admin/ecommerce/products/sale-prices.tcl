# $Id: sale-prices.tcl,v 3.0 2000/02/06 03:21:01 ron Exp $
# sale-prices.tcl
#
...
proc ec_write_out_one_sale {} {
	...
	"
	if { !$sale_begun_p } {
	    ns_write "<b>This sale has not yet begun.</b><br>\n"
	} elseif { $sale_expired_p } {
	    ns_write "<b>This sale has ended.</b><br>\n"
	}

	ns_write "\[<a href=\"sale-price-edit.tcl?[export_url_vars sale_price_id product_id product_name]\">edit</a>[ec_decode $sale_expired_p "0" " | <a href=\"sale-price-expire.tcl?[export_url_vars sale_price_id product_id product_name]\">expire now</a>" ""]"
	
	# Set audit variables
	# audit_name, id, id_column, return_url, audit_tables, main_tables
	set audit_name "$product_name Sale"
	set id $sale_price_id
	set id_column "sale_price_id"
	...
	ns_write " | <a href=\"/admin/ecommerce/audit.tcl?[export_url_vars audit_name id id_column return_url audit_tables main_tables]\">audit trail</a>\]
	<p>
	"
    }
}
...
ReturnHeaders
ns_write "[ad_admin_header "Sale Prices for $product_name"]
...
set selection [ns_db select $db "select sale_price_id, sale_name, sale_price, offer_code, to_char(sale_begins,'YYYY-MM-DD HH24:MI:SS') as sale_begins, to_char(sale_ends,'YYYY-MM-DD HH24:MI:SS') as sale_ends, decode(sign(sysdate-sale_begins),1,1,0) as sale_begun_p, decode(sign(sysdate-sale_ends),1,1,0) as sale_expired_p
...
if { $sale_price_counter == 0 } {
    ns_write "There are no current sale prices.\n"
}
...
<form method=post action=sale-price-add.tcl>
...
<tr>
<td>Sale Begins</td>
<td>[ad_dateentrywidget sale_begins] [ec_timeentrywidget sale_begins "x 00:00:00"]</td>
</tr>
<tr>
<td>Sale Ends</td>
<td>[ad_dateentrywidget sale_ends] [ec_timeentrywidget sale_ends "x 23:59:59"]</td>
</tr>
<tr>
...
if { $sale_price_counter == 0 } {
    ns_write "There are no non-current sale prices.\n"
}
