...
append doc_body "<b>Customer:</b> <a href=\"/admin/users/one?user_id=$user_id\">[db_string get_customer_info "select first_names || ' ' || last_name from users where user_id=:user_id"]</a>
...
db_foreach get_gift_certificates_for_user $sql {
	...
	append doc_body "<tr><td>Issued By</td><td><a href=\"/admin/users/one?user_id=$issuer_user_id\">$issuer</a></td></tr>"
    } else {
	append doc_body "<tr><td>Purchased By</td><td><a href=\"/admin/users/one?user_id=$purchaser_user_id\">$purchaser</a></td></tr>"
    }
	...
	append doc_body "<tr><td><font color=red>Voided</font></td><td>[util_AnsiDatetoPrettyDate $voided_date] by <a href=\"/admin/users/one?user_id=$voided_by\">$voided_by_first_names $voided_by_last_name</a> because: $reason_for_void</td></tr>"
    }
