...
if [empty_string_p $min_body_count] {
    set min_body_count NULL
}


set return_url "[edu_url]class/admin/projects/one.tcl?project_id=$project_id"
...
if {![empty_string_p $team_id]} {
    ...
    if { [database_to_tcl_string $db "select count(*) from edu_teams where team_id = $team_id"] > 0 } {
...
and team_id = $team_id"]                          == 0 } {
	...
	ad_returnredirect "[edu_url]class/admin/teams/create-3.tcl?[export_url_vars team_id team_name project_instance_id return_url]"
	return
    }
}


if {![empty_string_p $student_id] && $student_id > 0} {
    # we have a student for the project
    # we do the select to make sure that the user is a member of the class
    ns_db dml $db "insert into edu_project_user_map (project_instance_id, student_id) select $project_instance_id, $student_id from users, user_group_map map where users.user_id = $student_id and users.user_id = map.user_id and map.group_id = $class_id"
}
