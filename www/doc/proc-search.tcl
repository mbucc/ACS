ad_page_contract {
    ...
    @author ?
    @creation-date ?
    @cvs-id proc-search.tcl,v 3.5.2.3 2000/07/31 23:32:42 chiao Exp
} {
    query_string
    exclude_body_p:optional
}

ad_proc procs_tcl_sort_by_second_element {l1 l2} {} {
...
ad_proc procs_tcl_sort_by_second_element_desc {l1 l2} {} {
...
among <a href=\"procs\">the documented procedures</a> in this
...
foreach sublist $sorted_list {
	...
	# Prevent an error if the proc is not known.
	continue
    }
...
if { [llength $sorted_results] > 0 } {
	...
	ns_write "<li><a href=\"proc-search?exclude_body_p=1&[export_url_vars query_string]\">query again for \"$query_string\" but don't search through procedure bodies</a>\n<p>\n"
    }
    foreach sublist $sorted_results {
	set proc_name [lindex $sublist 0]
	set score [lindex $sublist 1]
	ns_write  "<li>$score: <b><a href=\"proc-one?proc_name=[ns_urlencode $proc_name]\">$proc_name</a></b> <i>[info args $proc_name]</i>\n"
    }
} else {
...
<li><a href=\"proc-search-all-procs?[export_url_vars query_string]\">query again for \"$query_string\" but this time search through all defined procedures, even system and undocumented procedures</a>\n<p>\n
