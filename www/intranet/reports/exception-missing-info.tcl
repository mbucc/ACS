...
set sql_query "select last_name || ', ' || first_names as name,
                      im_employee_info.user_id 
               from im_employee_info, users 
               where im_employee_info.user_id = users.user_id
               and im_employee_info.$exception_type is NULL
               and ad_group_member_p ( im_employee_info.user_id, [im_employee_group_id] ) = 't'
               order by lower(name)"

set context_bar [ad_context_bar_ws [list [im_url_stub]/reports/ Reports] "Exception Report"]
