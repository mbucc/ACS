#
# /www/education/util/upload-version.tcl
#
...
#
# modified by randyg@arsdigita.com to work with the permissions used by the
# education module
#
...
# one but not both of upload_file and url must be not null
...
# get the version_id for the item being replaced

set selection [ns_db 1row $db "select version_id as old_version_id from fs_versions_latest where file_id = $file_id"]


if {$selection == ""} {
    incr exception_count
    append exception_text "<li>The file you have provided does not exist."
} else {
    set_variables_after_query
}


if {! [fs_check_write_p $db $user_id $old_version_id $group_id]} {
    incr exception_count
...
if {[database_to_tcl_string $db "select count(version_id) from fs_versions where version_id = $version_id"] > 0} {
...
returning version_content into :1" 
    ...
    if {[catch { ns_ora blob_dml_file $db $version_insert $tmp_filename } errmsg] } {
        ...
        if { [database_to_tcl_string $db "select count(*) from fs_versions where version_id = $version_id"] == 0 } {
	    ns_log Error "[edu_url]group/admin/upload-version.tcl choked:  $errmsg"
...
# now that we are done with the insert, lets update the other file so that
# it knows that is have been superseded

ns_db dml $db  "update fs_versions 
...
##########################################
#                                        #
# Lets take care of the permissions here #
#                                        #
##########################################
...
fs_order_files $db $user_id $group_id f
