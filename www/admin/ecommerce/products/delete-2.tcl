# /www/admin/ecommerce/products/delete-2.tcl
ad_page_contract {
  Delete a product.
...
db_transaction {
  ...
  foreach offer_id $offer_list {
    ad_audit_delete_row [list $offer_id $product_id] [list offer_id product_id] ec_offers_audit
  }
  ...
  ad_audit_delete_row [list $product_id] [list product_id] ec_custom_p_field_values_audit
  ...
  foreach subsubcategory_id $subsubcategory_list {
    ad_audit_delete_row [list $subsubcategory_id $product_id] [list subsubcategory_id product_id] ec_subsubcat_prod_map_audit
  }
  ...
  foreach subcategory_id $subcategory_list {
    ad_audit_delete_row [list $subcategory_id $product_id] [list subcategory_id product_id] ec_subcat_prod_map_audit
  }
  ...
  foreach category_id $category_list {
    ad_audit_delete_row [list $category_id $product_id] [list category_id product_id] ec_category_product_map_audit
  }
  ...
  foreach review_id $review_list {
    ad_audit_delete_row [list $review_id $product_id] [list review_id product_id] ec_product_reviews_audit
  }
  ...
  foreach product_a $product_a_list {
    ad_audit_delete_row [list $product_a $product_id] [list product_a product_id] ec_product_links_audit
  }
  foreach product_b $product_b_list {
    ad_audit_delete_row [list $product_b $product_id] [list product_b product_id] ec_product_links_audit
  }
  ...
  foreach user_class_id $user_class_id_list {
    ad_audit_delete_row [list $user_class_id [lindex $user_class_price_list $counter] $product_id] [list user_class_id price product_id] ec_product_u_c_prices_audit
    incr counter
  }
  ...
  foreach series_id $series_id_list {
    ad_audit_delete_row [list $series_id $product_id] [list series_id component_id] ec_product_series_map_audit
  }
  foreach component_id $component_id_list {
    ad_audit_delete_row [list $product_id $component_id] [list series_id component_id] ec_product_series_map_audit
  }
  ...
  foreach sale_price_id $sale_price_list {
    ad_audit_delete_row [list $sale_price_id] [list sale_price_id] ec_sale_prices_audit
  }

  # 12. Products
  db_dml product_delete "delete from ec_products where product_id=:product_id"

  # audit
  ad_audit_delete_row [list $product_id] [list product_id] ec_products_audit
