...
if {$count_type == "new"} {
    ...
    set count_where_clause "im_employee_info.user_id in (select 
...
set current_job_id_info_sql "select distinct current_job_id as group_id, 
                              ...
                              from im_employee_info, im_job_titles
                             where im_employee_info.current_job_id = im_job_titles.job_title_id"
...
set source_info_sql "select distinct im_hiring_sources.source_id as group_id,
                      ...
                      from im_hiring_sources, im_employee_info
                     where im_employee_info.source_id = im_hiring_sources.source_id"
...
set experience_info_sql "select distinct im_prior_experiences.experience_id as group_id, 
                      ...
                      from im_prior_experiences, im_employee_info
                     where im_employee_info.experience_id = im_prior_experiences.experience_id"
...
set department_info_sql "select distinct im_departments.department_id as group_id, 
                      ...
                      from im_departments, im_employee_info
                     where im_departments.department_id = im_employee_info.department_id"
...
set report_count_sql "select min(start_block) as order_date, 
                             ...
                             $grouping_expr (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
                                                    from im_employee_percentage_time, im_employee_info
                                                   where start_block = imsb.start_block  [ad_dimensional_sql $dimensional where]
                                                     and im_employee_info.user_id = im_employee_percentage_time.user_id),0)) as total"
   ...
   set short_name short_name_$count
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
              from im_employee_percentage_time, user_group_map, im_employee_info
              where start_block = imsb.start_block  [ad_dimensional_sql $dimensional where]
                and user_group_map.user_id = im_employee_percentage_time.user_id
                and user_group_map.user_id = im_employee_info.user_id
                and im_employee_info.user_id = im_employee_percentage_time.user_id
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
               ...
               where start_block = imsb.start_block
                 ...
                 and im_employee_info.user_id = im_employee_percentage_time.user_id
                 and im_employee_info.department_id = $group_id),0))
        as [string tolower "a$short_name"]\n "
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
               ...
               where start_block = imsb.start_block
                     [ad_dimensional_sql $dimensional where]
                 and im_employee_info.user_id = im_employee_percentage_time.user_id
                 and im_employee_info.current_job_id = $group_id),0))
        as [string tolower "a$short_name"]\n "
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
               ...
               where start_block = imsb.start_block
                     [ad_dimensional_sql $dimensional where]
                 and im_employee_info.user_id = im_employee_percentage_time.user_id
                 and im_employee_info.original_job_id = $group_id),0))
        as [string tolower "a$short_name"]\n "
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
              ...
              where start_block = imsb.start_block
                    [ad_dimensional_sql $dimensional where]
                and im_employee_info.user_id = im_employee_percentage_time.user_id
                and im_employee_info.source_id = $group_id),0))
        as [string tolower "a$short_name"]\n "
	...
	set select_clause ",
        ...
        (nvl((select sum(((nvl(im_employee_percentage_time.percentage_time,100))*.01)) 
               ...
               where start_block = imsb.start_block   
                     [ad_dimensional_sql $dimensional where]
                 and im_employee_info.user_id = im_employee_percentage_time.user_id
                 and im_employee_info.experience_id = $group_id),0))
        as [string tolower "a$short_name"]\n "
    }
...
set context_bar [ad_context_bar_ws [list [im_url_stub]/reports/ Reports] "Employee Report"]
