# $Id: ecommerce-money-computations.tcl,v 3.0 2000/02/06 03:13:19 ron Exp $
# Computes the total price of an order.
# Note that it must have already had the price, shipping and tax filled in for each item
# and shipping, tax, and gift_certificate amount filled in for the order.
...
proc ec_lowest_price_and_price_name_for_an_item { db product_id user_id {offer_code ""} } {
    set lowest_price 0
    set lowest_price_name ""
    set reg_price [database_to_tcl_string $db "select price from ec_products where product_id=$product_id"]
    ...
    set selection [ns_db select $db "select p.price, c.user_class_name
	...
	set or_part_of_query "or offer_code='[DoubleApos $offer_code]'"
    } else {
	set or_part_of_query ""
    }
    set selection [ns_db select $db "select sale_price, sale_name
...
proc ec_shipping_price_for_one_item {db item_id product_id order_id shipping_method} {
    ...
    set selection [ns_db 1row $db "select shipping, shipping_additional, weight from ec_products where product_id=$product_id"]
    set_variables_after_query
	...
	set first_instance [database_to_tcl_string $db "select min(item_id) from ec_items where product_id=$product_id and order_id=$order_id"]
...
proc ec_price_price_name_shipping_price_tax_shipping_tax_for_one_item { db product_id offer_code item_id order_id shipping_method user_class_id_list default_shipping_per_item weight_shipping_cost add_exp_amount_per_item add_exp_amount_by_weight tax_rate shipping_p } {
...
proc ec_price_shipping_gift_certificate_and_tax_in_an_order { db order_id } {

    set selection [ns_db 1row $db "select confirmed_date, user_id from ec_orders where order_id=$order_id"]
    set_variables_after_query

    set total_price [database_to_tcl_string $db "select ec_total_price($order_id) from dual"]
    set total_shipping [database_to_tcl_string $db "select ec_total_shipping($order_id) from dual"]
    set total_tax [database_to_tcl_string $db "select ec_total_tax($order_id) from dual"]
	...
	set gift_certificate_balance [database_to_tcl_string $db "select ec_gift_certificate_balance([ns_dbquotevalue $user_id]) from dual"]
