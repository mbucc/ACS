...
set sql_query "select [ns_dbquotevalue $option_list_html] as option_list_from_db, u.first_names ||' '|| u.last_name as name, u.user_id as user_id from im_employee_info im_ei, users_active u
...
set sql_query "select [ns_dbquotevalue $option_list_html]  as option_list_from_db, 
               ...
               from im_employee_info im_ei, users_active u
               where im_ei.user_id = u.user_id
                     and 1 > (select count(*) from user_group_map, user_groups               
		              where user_group_map.user_id = im_ei.user_id 
...
set context_bar [ad_context_bar_ws [list [im_url_stub]/reports/ Reports] "Missing $group_type"]
