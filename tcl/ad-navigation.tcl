...
# hierarchy)
#
# $Id: ad-navigation.tcl,v 3.2 2000/02/21 10:34:24 ron Exp $
# -----------------------------------------------------------------------------

# the arguments are lists ( [list URL anchor])
# except for the last one, which we expect to be just text
proc_doc ad_context_bar args "Returns a Yahoo-style hierarchical navbar, each arg should be a list of URL and description.  The last arg should be a just plain description." {
    set choices [list]
    set index 0
    foreach arg $args {
	incr index
...
proc menu_uplevel {section  {uplevel_link ""}} {
	...
	if {[file tail [ns_conn url]] == [lindex [menu_section_uplevel_list $section] 0]} {
	    ...
	    set fileroot [ns_info pageroot]$urlroot
	} else {
