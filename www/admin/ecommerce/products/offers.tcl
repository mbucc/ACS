#  www/admin/ecommerce/products/offers.tcl
ad_page_contract {

  @author
  @creation-date
  @cvs-id offers.tcl,v 3.1.6.4 2001/01/12 18:26:39 khy Exp
} {
...
proc ec_write_out_one_offer {} {
	...
	if { ![empty_string_p $stock_status] } {
	    doc_body_append "[ad_parameter "StockMessage[string toupper $stock_status]" ecommerce]<br>\n"
	} else {
...
select o.offer_id, erl.retailer_id, r.retailer_name, price, shipping,
...
from ec_offers_current o, ec_retailers r, ec_retailer_locations erl
where o.retailer_location_id = erl.retailer_location_id 
and erl.retailer_id = r.retailer_id
...
select o.offer_id, erl.retailer_id, retailer_name, price, shipping,
       stock_status, special_offer_p, special_offer_html,
       shipping_unavailable_p, offer_begins, offer_ends, o.deleted_p,
       decode(sign(sysdate-offer_begins),1,1,0) as offer_begun_p,
       decode(sign(sysdate-offer_ends),1,1,0) as offer_expired_p
from ec_offers o, ec_retailers r, ec_retailer_locations erl
where o.retailer_location_id=erl.retailer_location_id
    and r.retailer_id = erl.retailer_id
    and o.product_id=:product_id
