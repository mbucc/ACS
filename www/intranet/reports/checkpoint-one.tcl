...
set table_def {
    {employee "Employee Info" no_sort {<td><a href=[im_url_stub]/employees/admin/pipeline-ae?user_id=$user_id>$name</a>}}
}
set sql_query "select first_names||' '||last_name as name, im_employee_info.user_id as user_id 
                from im_emp_checkpoint_checkoffs, im_employee_info, users
                 where im_employee_info.user_id = users.user_id
                 and im_emp_checkpoint_checkoffs.checkee(+) = im_employee_info.user_id
...
set context_bar [ad_context_bar_ws [list "[im_url_stub]/reports/" "Reports"] [list "[im_url_stub]/reports/checkpoint-progress" "Checkpoints"] "One CheckPoint"]
