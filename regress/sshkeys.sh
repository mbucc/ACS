...
# Copy over the same SSH key for the oracle and nsadmin users.
...
ssh \
  root@${ACS_IP_ADDRESS} -i $ROOTKEY \
  "(mkdir -p $ORACLE_USER_HOME/.ssh; chown $ORACLE_USER:$ORACLE_GROUP $ORACLE_USER_HOME/.ssh)"
echo "Copying over public key ..."
scp -i $ROOTKEY ./oracle_id_rsa.pub root@${ACS_IP_ADDRESS}:$ORACLE_USER_HOME/.ssh/authorized_keys
...
echo "Copying over public key ..."
scp -i $ROOTKEY ./oracle_id_rsa.pub root@${ACS_IP_ADDRESS}:$AOL_HOME/.ssh/authorized_keys
echo "Setting ownership of public key ..."
ssh \
  root@${ACS_IP_ADDRESS} -i $ROOTKEY \
