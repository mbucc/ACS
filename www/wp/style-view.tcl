# $Id: style-view.tcl,v 3.2 2000/03/10 20:43:58 jsalz Exp $
# File:        style-view.tcl
# Date:        28 Nov 1999
# Author:      Jon Salz <jsalz@mit.edu>
# Description: Allows the user to view a style.
...
ns_write "[wp_header_form "name=f action=style-image-add.tcl method=post enctype=multipart/form-data" \
           [list "" "WimpyPoint"] [list "style-list.tcl?user_id=$user_id" "Your Styles"] $name]
    ...
    <td colspan=5>
      <table border=2 $bgcolor_str cellpadding=10>
        <tr><td $bgimage_str>
          ...
          ${alink_color_font}<u>Linked Text (Visited)</u>$alink_color_font_end<br>
          ${vlink_color_font}<u>Linked Text (Active)</u>$vlink_color_font_end
        </td></tr>
      </table>
    </td>
  <tr>
    <th nowrap align=right>CSS Code:&nbsp;</th>
      <td colspan=5>[expr { [regexp {[^ \n\r\t]} $css] ? "<a href=\"css-view.tcl?style_id=$style_id\">view</a>" : "none" }]</td>
  </tr>
  <tr>
    <td align=center colspan=5><br><input type=button onClick=\"location.href='style-edit.tcl?style_id=$style_id'\" value=\"Edit Style\"><hr></td>
  </tr>
"
...
<td align=right><a href=\"style-image-delete.tcl?style_id=$style_id&file_name=[ns_urlencode $file_name]\">delete</a></td>
</tr>
"
} else {
