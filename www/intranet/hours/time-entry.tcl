# /www/intranet/hours/time-entry.tcl
...
<input type=hidden name=\"old_hours.${group_id}.${julian_date}.notes\" [export_form_value notes]>
    Hours: &nbsp; <input type=text name=hours.${group_id}.${julian_date}.hours size=5 [export_form_value hours]>
    <br>Notes:<br><input type=text name=hours.${group_id}.${julian_date}.notes size=15 [export_form_value notes]>
  </td>
"
}
...
if { [empty_string_p $julian_date] } {
    ...
    set julian_date [db_string julian_date_from_daul \
...
if { ![empty_string_p $on_what_id] } {
    ...
    set project_id_list [db_list project_id_list \
	    "select map.group_id 
               from user_group_map map, im_projects p
              where map.user_id = $user_id
                and map.group_id = p.group_id
...
set bgcolor(0) " bgcolor=\"[ad_parameter TableColorOdd intranet "ffffff"]\""
set bgcolor(1) " bgcolor=\"[ad_parameter TableColorEven intranet "cccccc"]\""
...
<tr>
  <td colspan=8 bgcolor=#dddddd align=center>
    <a href=other-projects?[export_url_vars user_id on_which_table julian_date]>Add hours on other projects</A>
  </td>
</tr>
"
}
...
set page_title "Hours for $user_name (Week starting [util_AnsiDatetoPrettyDate $ansi_date])"
set context_bar [ad_context_bar_ws [list index?[export_url_vars on_which_table] "Hours"] "Add hours"]

set options [list "<a href=index?[export_ns_set_vars url [list julian_date]]>Log hours for a different week</a>"]
