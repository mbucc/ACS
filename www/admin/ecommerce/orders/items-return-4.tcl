# $Id: items-return-4.tcl,v 3.0.4.1 2000/04/28 15:08:45 carsten Exp $
# items-return-4.tcl
# 
# by eveander@arsdigita.com, July 22, 1999
#
...
# cash_amount_to_refund, certificate_amount_to_reinstate

# possibly card_query_p, card_query_password,
...
if { [expr $cash_amount_to_refund] > 0 && [empty_string_p [database_to_tcl_string $db "select creditcard_number from ec_orders o, ec_creditcards c where o.creditcard_id=c.creditcard_id and o.order_id=$order_id"]]} {
...
if { [expr $cash_amount_to_refund] > 0 } {
	...
	set transaction_to_query [database_to_tcl_string $db "select max(transaction_id) from ec_financial_transactions where creditcard_id=$creditcard_id and (authorized_date is not null OR 0=(select count(*) from ec_financial_transactions where creditcard_id=$creditcard_id and authorized_date is not null)"]
	...
	if { [empty_string_p $creditcard_number] } {
	    ...
	    # this is where we begin the transaction for Case A (card-query to get credit card number)

	    ns_db dml $db "begin transaction"

	    # update the credit card number in ec_creditcards
	    ns_db dml $db "update ec_creditcards set creditcard_number='[DoubleApos $creditcard_number]' where creditcard_id=$creditcard_id"
	}
    } elseif { [info exists card_query_p] && $card_query_p == "f" } {
	# this is where we begin the transaction for Case B (insert new credit card number)
	
	ns_db dml $db "begin transaction"
	
	# insert a new credit card into ec_creditcards
	set creditcard_id [database_to_tcl_string $db "select creditcard_id_sequence.nextval from dual"]
	...
	# this is where we begin the transaction for Case C (credit card number still in database)
	ns_db dml $db "begin transaction"
    }
} else {
    # this is where we begin the transaction for Case D (no money has to be refunded)
    ns_db dml $db "begin transaction"
}
...
ns_db dml $db "insert into ec_refunds
(refund_id, order_id, refund_amount, refund_date, refunded_by, refund_reasons)
values
($refund_id, $order_id, $cash_amount_to_refund, sysdate, $customer_service_rep,'$QQreason_for_return')
"
...
foreach item_id $item_id_list {
    ...
    set selection [ns_db 1row $db "select nvl(price_tax_charged,0) as price_tax_charged, nvl(shipping_tax_charged,0) as shipping_tax_charged from ec_items where item_id=$item_id"]
    set_variables_after_query

    set price_tax_to_refund [ec_min $price_tax_charged [database_to_tcl_string $db "select ec_tax($price_to_refund($item_id),0,$order_id) from dual"]]
    set shipping_tax_to_refund [ec_min $shipping_tax_charged [database_to_tcl_string $db "select ec_tax(0,$shipping_to_refund($item_id),$order_id) from dual"]]

    ns_db dml $db "update ec_items set item_state='received_back', received_back_date=to_date('$received_back_date','YYYY-MM-DD HH24:MI:SS'), price_refunded=[ns_dbquotevalue $price_to_refund($item_id)], shipping_refunded=[ns_dbquotevalue $shipping_to_refund($item_id)], price_tax_refunded=[ns_dbquotevalue $price_tax_to_refund], shipping_tax_refunded=[ns_dbquotevalue $shipping_tax_to_refund], refund_id=$refund_id where item_id=$item_id"
}

set base_shipping_tax_charged [database_to_tcl_string $db "select nvl(shipping_tax_charged,0) from ec_orders where order_id=$order_id"]
set base_shipping_tax_to_refund [ec_min $base_shipping_tax_charged [database_to_tcl_string $db "select ec_tax(0,$base_shipping_to_refund,$order_id) from dual"]]

ns_db dml $db "update ec_orders set shipping_refunded=[ns_dbquotevalue $base_shipping_to_refund], shipping_tax_refunded=[ns_dbquotevalue $base_shipping_tax_to_refund] where order_id=$order_id"
...
if { $certificate_amount_to_reinstate > 0 } {
    ...
    set selection [ns_db select $db "select u.gift_certificate_id, c.amount as original_amount
	...
	foreach cert_and_original_amount $certs_to_reinstate_list {
		...
		set reinstatable_amount [expr $original_amount - [database_to_tcl_string $db "select gift_certificate_amount_left($cert) from dual"]]
...
if { $cash_amount_to_refund > 0} {
    ...
    set refund_status [ec_creditcard_return $db $transaction_id]
    if { $refund_status == "failure" || $refund_status == "invalid_input" } {
	ns_db dml $db "insert into ec_problems_log
	(problem_id, problem_date, problem_details, order_id)
	values
	(ec_problem_id_sequence.nextval, sysdate, 'When trying to refund refund_id $refund_id (transaction $transaction_id) at [DoubleApos [ns_conn url]], the following result occurred: $refund_status', $order_id)
	"
...
$results_explanation
<p>
<a href=\"one.tcl?[export_url_vars order_id]\">Back to Order $order_id</a>
</blockquote>
